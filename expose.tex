...
SO in the case of a point, $St_X$ can be identified with the geometric realization functor $\vert\cdot\vert_{Q^\cdot}:\SSet\rightarrow \SSet$ for some cosimplicial object $Q^\cdot$ in $\SSet$. Let's describe $Q^\cdot$ explicitly. Let's describe a cosimplicial simplicial set $J^\cdot$ by the formula \[J^n = \displaystyle (\Delta^n\star \{y\})\cup_{\Delta^n}\{x\}.\]
The cosimplicial simplicial set $Q^\cdot$ can then be described by the formula $Q^n=\Map_{\mathfrak{C}[J^n]}(x,y)$.\\
$Q^\cdot$ admits the following description: for each $n\geq 0$, let $P_[n]$ denote the partially ordered set of nonempty subsets of $[n]$, and $K_{[n]}$ the simplicial set $N(P)$ (which may be identified with a simplicial subset of the $(n+1)$-cube $(\Delta^1)^{n+1}$. The simplicial set $Q^n$ is obtained by collapsing, for each $0\leq i\leq n$, the subset  $(\Delta^1)^{\{j:0\leq j<i\}\times \{1\}\times (\Delta^1)^{\{j:i<j\leq n\}\subset K_{[n]}$ to its quotient $(\Delta^1)^{\{j:i<j\leq n\}}\subset K_{[n]}$.
...
In other words, if we look at $\mathscr{Q}$, the geometric realization of $Q^\cdot$, then $\mathscr{Q}^n$ is a quotient of the cube of all functions $p:[n]\rightarrow[0,1]$ satisfying $p(0) = 1$. We quotient by the following relation: $p\simeq p'$ if there exists an integer $0\leq i\leq n$ such that $p_{\vert\{i,\dots,n\}} = p'_{\vert\{i,\dots,n\}}$ and $p(i) = p'(i) = 1$. SO each $\mathscr{Q}^n$ is homeomorphic to an $n$-simplex in a way that's compatible with the face maps of the cosimplicial space $\mathscr{Q}^\cdot$. However, the degeneracies are very different from the degeneracies of the standard simplex. For instance, the product of the degeneracy mapping $\mathscr{Q}^n\rightarrow (\mathscr{Q}^1)^n$ is not injective for $n\geq 2$.\\
Now that we have a better understanding of $Q^\cdot$, let's study the functors $Sing_{Q^\cdot}$ and $\vert \cdot \vert_{Q^\cdot}$. There is a map $\pi:Q^\cdot\rightarrow\Delta^\cdot$ of cosimplicial objects of $\SSet$. It is induced by a map $K_{[n]}\rightarrow \Delta^n$, which is the nerve of the map of partially ordered sets $P_{[n]}\mapsto[n]$ that carries each nonempty subset of $[n]$ to its largest element. 

\begin{proposition}
  Let $S$ be a simplicial set. The map $p_S:\vert S\vert_{Q^\cdot}\rightarrow S$ induced by $\pi$ is a weak homotopy equivalence.
\end{proposition}
...
First, let's look at what happens when $S$ is a point and $\phi$ is an isomorphism of simplicial categories. In this case, the straightening functor $St^+_{\Delta^0}$ is actually the functor $T:\SSet^+\rightarrow \SSet^+$ defined by: \[T(X,\mathcal{E}) = (\vert X\vert_{Q^\cdot},\mathcal{E}'),\] where $Q$ is the specific cosimplicial object of $\SSet$ defined in the unmarked case.

Then $T$ has a right adjoint $U$, and the pair $(T,U)$ is a Quillen equivalence from $(\SSet)^+$ to itself.
Now, if we return to the case of a general equivalence $\phi:\mathfrak{C}[S]\rightarrow \C^{op}$, we know we have a Quillen adjunction, so to prove the equivalence, we want to show that the unit and counit 
\begin{align*}
  u:id &\rightarrow RUn_\phi^+\circ LSt_\phi^+\\
  v:LSt_\phi^+\circ RUn_\phi^+&\rightarrow id
\end{align*}
