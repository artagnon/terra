...
\usepackage[a4paper, inner=25mm, outer=25mm]{geometry}
...
Let's look at the simplicial set $X = \{x\}$. In this case, we can view $St_X$ as a colimit-preserving functor from $\SSet$ to itself. Let's look at the specific properties of such a functor: if \C is a category that admits small colimits, then any functor $f:\Delta \rightarrow \C$ extends to a colimit-preserving functor $F:\SSet\rightarrow \C$. So the functor $F$ is represented by a cosimplicial object, which we take to be $f$: $F:S\mapsto \vert S\vert_{C^\dot}$ (defined by taking a coend). The functor $S\mapsto\vert S\vert_{C^\dot}$ has a right adjoint that we denote by $Sing_{C^\dot}$ and that we describe by the formula:
\[Sin_{C^\dot}(X)_n = \Hom_\C(C^n,X).\] . 

SO in the case of a point, $St_X$ can be identified with the geometric realization functor $\vert\dot\vert_{Q^\dot}:\SSet\rightarrow \SSet$ for some cosimplicial object $Q^\dot$ in $\SSet$. Let's describe $Q^\dot$ explicitly. Let's describe a cosimplicial simplicial set $J^\dot$ by the formula \[J^n = \displaystyle (\Delta^n\star \{y\})\cup_{\Delta^n}\{x\}.\]
The cosimplicial simplicial set $Q^\dot$ can then be described by the formula $Q^n=\Map_{\mathfrak{C}[J^n]}(x,y)$.\\
$Q^\dot$ admits the following description: for each $n\geq 0$, let $P_[n]$ denote the partially ordered set of nonempty subsets of $[n]$, and $K_{[n]}$ the simplicial set $N(P)$ (which may be identified with a simplicial subset of the $(n+1)$-cube $(\Delta^1)^{n+1}$. The simplicial set $Q^n$ is obtained by collapsing, for each $0\leq i\leq n$, the subset  \[(\Delta^1)^{\{j:0\leq j<i\}\times \{1\}\times (\Delta^1)^{\{j:i<j\leq n\}\subset K_{[n]}\] to its quotient $(\Delta^1)^{\{j:i<j\leq n\}}\subset K_{[n]}$ to its quotient $(\Delta^1)^{\{J:i<j\leq n\}}$\\
A map $f:[n]\rightarrow[m]$ determines a map $P_f:P_{[n]}\rightarrow P_{[m]}$ by setting $P_f(I) = f(I)$. The map $P_f$ in turn induces a map of simplicial sets $K_{[n]}\rightarrow K_{[m]}$, whcih determines a map of quotients $Q^n\rightarrow Q^m$ when $f$ is order-preserving. 
In other words, if we look at $\mathscr{Q}$, the geometric realization of $Q^\dot$, then $\mathscr{Q}^n$ is a quotient of the cube of all functions $p:[n]\rightarrow[0,1]$ satisfying $p(0) = 1$. We quotient by the following relation: $p\simeq p'$ if there exists an integer $0\leq i\leq n$ such that $p_{\vert\{i,\dots,n\}} = p'_{\vert\{i,\dots,n\}}$ and $p(i) = p'(i) = 1$. SO each $\mathscr{Q}^n$ is homeomorphic to an $n$-simplex in a way that's compatible with the face maps of the cosimplicial space $\mathscr{Q}^\dot$. However, the degeneracies are very different from the degeneracies of the standard simplex. For instance, the product of the degeneracy mapping $\mathscr{Q}^n\rightarrow (\mathscr{Q}^1)^n$ is not injective for $n\geq 2$.\\
Now that we have a better understanding of $Q^\dot$, let's study the functors $Sing_{Q^\dot}$ and $\vert \dot \vert_{Q^\dot}$. There is a map $\pi:Q^\dot\rightarrow\Delta^\dot$ of cosimplicial objects of $\SSet$. It is induced by a map $K_{[n]}\rightarrow \Delta^n$, which is the nerve of the map of partially ordered sets $P_{[n]}\mapsto[n]$ that carries each nonempty subset of $[n]$ to its largest element. 

\begin{proposition}
  Let $S$ be a simplicial set. The map $p_S:\vert S\vert_{Q^\dot}\rightarrow S$ induced by $\pi$ is a weak homotopy equivalence.
\end{proposition}
...
\begin{proof}
  ...
  $$\vert S\vert_{Q^\dot}\simeq \vert S'\vert_{Q^\dot}\displaystyle \cup_{\vert \partial \Delta^n\vert_{Q^\dot}}\vert\Delta^n\vert_{Q^\dot}.$$
  These pushouts are homotopy pushouts, so all we have to do is prove that $p_{S'}, p_{\partial\Delta^n},$, and $p_{\Delta^n}$ are weak homotopy equivalences. This follows from the inductive hypothesis for $p_{S'}$ and $p_{\partial\Delta^n}$. For $p_{\Delta^n}$, it is true because $\Delta^n$ and $\vert\Delta^n\vert_{Q^\dot}=\mathscr{Q}^n$ are weakly contractible.
\end{proof}

\begin{proposition}
  The adjoint functors $\SSet\adj{\vert \dot \vert_{Q^\dot}}{Sing_{Q^\dot}}\SSet$ determine a Quillen equivalence from the category $\SSet$ to itself.
\end{proposition}

This directly entails the following proposition:

\begin{proposition}
  Let $X$ be a Kan complex. Then the counit map\[v:\vert Sing_{Q^\dot}X\vert_{Q^\dot}\rightarrow X\] is a weak homotopy equivalence.
\end{proposition}

Let $S$ be a simplicial set containing a vertex $s$. Let $\C$ be a simplicial category, $\phi:\mathfrak{C}[S]^{op}\rightarrow \C$ a simplicial functor, and $C=\phi(s)\in\C$. For every simplicial functor $\mathcal{F}:\C\rightarrow \SSet$, there is a canonical isomorphism \[(Un_\phi\mathcal{F})\times_S\{s\}\simeq \Sing_{Q^\dot}\mathcal{F}(C).\]
...
First, let's look at what happens when $S$ is a point and $\phi$ is an isomorphism of simplicial categories. In this case, the straightening functor $St^+_{\Delta^0}$ is actually the functor $T:\SSet^+\rightarrow \SSet^+$ defined by: \[T(X,\mathcal{E}) = (\vert X\vert_{Q^\dot},\mathcal{E}'),\] where $Q$ is a specific cosimplicial object of $\SSet$ defined in the unmarked case.
