...
We start with the definition of the never functor.

\begin{definition}[Simplicial nerve]
  Given a small category $\C$ a simplex category $\boldsymbol\Delta$, and a map $i : \boldsymbol\Delta \rightarrow \C$, there exists a functor $N : \C \rightarrow \Set^{\op{\Delta}}$, or $\C \hookrightarrow \SSet$ called the nerve of the category, which is a simplicial set associated to the category. Applying Yoneda lemma to the Yoneda embedding $N$ yields:
...
\begin{definition}[Left fibration]
  A morphism $f:X\rightarrow S$ is a left fibration if $f$ has the right lifting property with respect to the horn inclusions $\Lambda_i^n\subset \Simplex{n}, 0\leq i < n$.
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item For every morphism $\eta: C \rightarrow C'$ in $\C$ and every object $C'' \in \C$, the map
          $$
            Hom_\C(C',C'')\rightarrow Hom_\C(C,C'') \times_{Hom_\D(F(C),F(C''))} Hom_\D(F(C'),F(C'')))
          $$
          is bijective.
  \end{itemize}
\end{definition}
