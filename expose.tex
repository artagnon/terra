...
\begin{definition}
  Given a small category $\mathscr{C}$ a simplex category $\Delta$, and a map $i : \Delta \rightarrow \mathscr{C}$, there exists a functor $N : \mathscr{C} \rightarrow \Set^{\op{\Delta}}$, or $\mathscr{C} \hookrightarrow \SSet$ called the nerve of the category, which is a simplicial set associated to the category. Applying Yoneda lemma to the Yoneda embedding $N$ yields:

  $$
    N(\mathscr{C})_i = Hom(\Simplex{n}, N(\mathscr{C}))
  $$
\end{definition}
...
This simplicial nerve will play a crucial role in studying $\8$-categories.

We now define quasi-categories, or what will henceforth referred to as $\8$-categories.
