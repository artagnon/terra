...
\begin{frame}{$S^\sharp$ and $S^\flat$}
  ...
  \onslide<4>{We will soon introduce the cartesian model structure on $(\SSet^+)_{/S}$, and in particular see that each $(\SSet^+)_{/S}$ is a simplicial model category whose fibrant objects are the cartesian fibrations $X \rightarrow S$.}
\end{frame}
